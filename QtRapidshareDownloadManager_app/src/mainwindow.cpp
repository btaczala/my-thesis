	...
	m_RapidshareDownloadManager( NULL ), m_Logger(" mainwindow "), m_ContextMenuOnItem(0), m_qpStatusBar(new QStatusBar(this)), m_qpProgressBarInStatusBar(new QProgressBar(m_qpStatusBar)), m_qpConnectionText(new QLabel(m_qpStatusBar)), m_qpConnectionIndicator(new QLabel(m_qpStatusBar)),m_qpTestConnection( new QPushButton(m_qpStatusBar))
{
	...
	*/
	...
	{
		QString progress = QString::number( pItem->GetProgress() );
		m_DownloadView->AddToDownload(pItem->GetFullUrlFileAddress(), pItem->GetFileDestination(), QString::number(pItem->GetProgress()),tr(""),StateToString2(pItem->GetState()));
	}
};
...
void MainWindow::Slot_EditMenu_Resume()
{
	...
	foreach( QTreeWidgetItem * pItem, selected ) 
	{
		...
		m_RapidshareDownloadManager->resumeAt(iPos);
	}
};
