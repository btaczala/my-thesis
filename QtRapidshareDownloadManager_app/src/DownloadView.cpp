...
void DownloadView::AddToDownload( const QString & urlPath, const QString & fileDest, const QString & progress, const QString & downloadRate, const QString & state )
{
	DownloadItem *pItem = new DownloadItem() ; 
	pItem->setText (UrlPathColumnID, urlPath);
	pItem->setText( FilePathColumnID, fileDest ) ;
	pItem->setText( ProgressColumnID, progress ) ;
	pItem->setText( DownloadRateColumnID, downloadRate ) ;
	pItem->setText( StatusColumnID, state ) ;
	setCurrentItem( pItem ) ;
	addTopLevelItem( pItem ) ;
}
