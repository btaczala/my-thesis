...
public:
	...
	*/
	...
	void							resumeAt(const unsigned int & at ) ;
	/*!
		Stop All - 
	*/
	void							stopAll();
	...
	void							LoadQueue();
	void							SaveQueue();
	const QList<QRapidshareDownload *> GetDownloadList() ; 

	bool							FindDownloadWithUrl(const QString & findUrl ) ;
	void							UpdateInSettings(const int & position, const QString & what, const QString & value ) ; 
